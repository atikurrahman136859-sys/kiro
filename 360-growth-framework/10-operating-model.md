...
```mermaid
flowchart TB
    subgraph DEMAND["1. DEMAND ENGINE"]
        MKT["Own marketing\n+ referrals + case studies"]
        SALES["Roadmap-first selling\n(project + retainer)"]
    end
    subgraph DELIVERY["2. DELIVERY ENGINE"]
        PODS["Client pods"]
    ...
    subgraph GROWTH["4. GROWTH ENGINE"]
        QBR["QBRs + maturity model"]
        RETAIN["Retention + expansion"]
    end
    DEMAND --> DELIVERY --> GROWTH
    INFRA -. powers everything .-> DEMAND
    INFRA -. powers everything .-> DELIVERY
    INFRA -. powers everything .-> GROWTH
    GROWTH -->|expansion + advocacy| DEMAND
```
