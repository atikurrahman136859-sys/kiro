...
```mermaid
flowchart LR
    L1["Project buyer\n(one service)"] --> L2["Repeat buyer\n(more projects)"]
    L2 --> L3["Retainer client\n(recurring)"]
    L3 --> L4["Multi-service partner\n(several lines)"]
    L4 --> L5["Growth partner\n(strategic, multi-year)"]
    L5 --> L6["Advocate\n(referrals + case study)"]
```
...
```mermaid
flowchart LR
    ACQUIRE["Acquire"] --> ONBOARD["Onboard\n(first value fast)"]
    ONBOARD --> ENGAGE["Engage\n(lifecycle comms)"]
    ENGAGE --> REWARD["Reward\n(loyalty)"]
    REWARD --> ADVOCATE["Advocate\n(referrals/reviews)"]
    ADVOCATE --> ACQUIRE
    ENGAGE -. detect risk .-> RECOVER["Recover\n(win-back)"]
    RECOVER --> ENGAGE
```
